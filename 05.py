...
while running:
    ...
    for i in range(len(balls)):
        if balls[i].color == BALL_COLOR_WHITE:
            pygame.draw.circle(screen, balls[i].color, (int(balls[i].x), int(balls[i].y)), 20)
        else:
            pygame.draw.circle(screen, balls[i].color, (int(balls[i].x), int(balls[i].y)), 50)
